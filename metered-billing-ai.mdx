...
<Note>
**Sub-cent Pricing (e.g., $0.005 per token)**
Because the API accepts prices in integer cents (`unit_amount`), you cannot directly specify $0.005 (which is half a cent). Instead, you must package your credits into blocks. For example, to charge $0.005 per credit, price it as **$5.00 per 1,000 credits** by setting `unit_amount: 500` and configuring a `transform_quantity` block to divide usage by 1,000.
</Note>
