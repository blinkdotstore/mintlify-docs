...
By default, the embedded checkout will automatically detect the host website's theme (light or dark mode) and apply the correct backdrop and checkout UI seamlessly. This overrides the store's default "Overlay Theme Mode" setting. 

If you want to manually force a specific theme, or disable auto-detection, you can pass an `options` object:

```javascript
// Force dark mode
window.Blink.openCheckout('https://...', { theme: 'dark' });

// Force light mode
window.Blink.openCheckout('https://...', { theme: 'light' });

// Disable auto-detection (respects the store's default setting)
window.Blink.openCheckout('https://...', { theme: 'none' });
```
